...
/*Copyright 2012 Wenjun Zheng*/
/*For any inquiry or licence, contact him.*/
...
void showMap(int* seqAA1, int* seqAA2, int len1, int len2,int* map)
{
  ...
  if (BLAST_DEBUG)
    {
      fprintf(stdout,"Sequence Alignment as shown below:\n");
...
int*    BLAST2seq_char (char* name, char* seq1, char* seq2, int len1, int len2, int* length_mapped)
{
  ...
  /* call bl2seq */
  //  sprintf(line,"/Users/mt/Documents/ccr_home/ncbi-blast-2.2.28+/bin/./legacy_blast.pl bl2seq -i seq1_%s -j seq2_%s -p blastp -o out_blast_%s -F F -e 0.001 > junk",name,name,name);  /* default E=0.001 */
  sprintf(line,"/Users/mt/Documents/ccr_home/ncbi-blast-2.2.28+/bin/blastp -query seq1_%s -subject seq2_%s > out_blast_%s ",name,name,name);  
  ...
  while (!feof(fid1)) {
    ...
    if (BLAST_DEBUG)   fprintf(stdout,"Q: %s %d-%d\n",s1,ptr1[0],ptr1[1]);
    if (BLAST_DEBUG)   fprintf(stdout,"   %s\n",sim);
    if (BLAST_DEBUG)   fprintf(stdout,"S: %s %d-%d\n\n",s2,ptr2[0],ptr2[1]);
    ...
    for (j=0;j<strlen(s1);++j) {
      if ((s1[j]!='-')&&(s2[j]!='-')) {
	if ((sim[j]=='+')||(s1[j]==s2[j]))	/* truly similar */
	  map[ptr1[0]-1]=ptr2[0]-1; 
	else if (0)				/* just aligned  */
	  map[ptr1[0]-1]=ptr2[0]-1;
	ptr1[0]+=1;
	ptr2[0]+=1;
	if ((s1[j]==s2[j])&&(s1[j]!='X'))	length_mapped[0]+=1;
      } else if ((s1[j]!='-')&&(s2[j]=='-')) {
	ptr1[0]+=1;
      } else if ((s1[j]=='-')&&(s2[j]!='-')) {
	ptr2[0]+=1;
  ...
  if (BLAST_DEBUG) 
    printf("Sequence alignment: Map sequence 1(%d) to sequence 2(%d) with %d residues matched (identity=%3.2f)\n", len1, len2, length_mapped[0], 1.0*length_mapped[0]/len1);
...
{                         /* aa #heavy-atom #all-atom */
  ...
  if (0&&BLAST_DEBUG) fprintf(stdout,"Warning:%s is unknown AA\n",s);
...
int blastAll(char *name, CAcoord *atom1, CAcoord *atom2, int lengthProtein1, int lengthProtein2)
{
  ...
  for(i=0;i<lengthProtein1;i++)
    {
      //    map[i]=i; //This is meaningless! If you dont want to map, why do you call this functions? 
      if(0)
	printf("map[%d]=%d\n",i, map[i]);
    }
  ...
  printf("Number of residues mapped -1 =%d\n", j);
  fclose(CAcoord1);
  fclose(CAcoord2);
  fclose(residueInfo1);
  fclose(residueInfo2);
...
double blastAll_v2(char *name, CAcoord *atom1, CAcoord *atom2, int lengthProtein1, int lengthProtein2)
{
  /*Purpose: Align two sequences and write */
  int i=0;
  ...
  /*   printf("map[%d]=%d\n",i, map[i]); */
  /*   } */
  double identity=(1.0*length_mapped/lengthProtein1);
  printf("Number of residues mapped -2=%d\n", length_mapped);
  printf("Identity for %s is  %3.2lf.\n", name, identity);
  free(seqAA1);  
  free(seqAA2);  
  return (identity);
}

/* int*    BLAST2seq_char_v2(char* name, char*    seq1, char*    seq2, int len1, int len2, int* length_mapped) */
/* { */
/*   /\*Purpose: Call bl2seq to align two sequences *\/ */
/*   /\*Whats new: In Dr. Zheng's version, you can just see gaps if they are not aligned.  */
/*                In my version, I am trying to see mutated residues, equivalent mutations */
/* 	       and gaps all together. *\/ */
/*   //If map[i]=(-1) : Gap */
/*   //If map[i]=( 0) : Same residue */
/*   //If map[i]=(+1) : Equivalent positive */
/*   //NOT FINISHED YET!!!!! */
...
/*   return (map); */
/* } */
int *blastAll_v3(CAcoord *atomSet1, CAcoord *atomSet2, int lengthProtein1, int lengthProtein2, \
		char *file1, char *file2, int printDetails, int printMap2File)
{
  ...
  for(i=0;i<lengthProtein1;i++)
    seqAA1[i]=aa2int(atomSet1[i].residname);

  for(i=0;i<lengthProtein2;i++)
    seqAA2[i]=aa2int(atomSet2[i].residname);
  ...
  if(printMap2File)
      ...
      for(i=0;i<lengthProtein1;i++)
	{
	  if(map[i]!=(-1))
	    {
	      fprintf(residueInfo1,"ATOM  %5d %s %3.3s %c%4d    %8.3lf%8.3lf%8.3lf%6.2lf%6.2lf\n" , (i+1), " CA ", \
		      atomSet1[i].residname,  atomSet1[i].chain, atomSet1[i].resNo, \
		      atomSet1[i].x,  atomSet1[i].y, atomSet1[i].z, 1.00, 0.00);
	      
	      fprintf(residueInfo2, "ATOM  %5d %s %3.3s %c%4d    %8.3lf%8.3lf%8.3lf%6.2lf%6.2lf\n", (map[i]+1), " CA ", \
		      atomSet2[map[i]].residname,  atomSet2[map[i]].chain, atomSet2[map[i]].resNo, \
		      atomSet2[map[i]].x,  atomSet2[map[i]].y, atomSet2[map[i]].z, 1.00, 0.00);
	    }
	}
      fclose(residueInfo1);
      fclose(residueInfo2);
  ...
  if(printDetails)  
    printf("Number of residues mapped -3=%d\n", j);
  return (map);
}
